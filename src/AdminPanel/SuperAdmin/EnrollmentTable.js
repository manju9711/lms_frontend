  ...
  return (
    <div className="p-4">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        {loading && <div className="text-xs text-gray-500">Loading enrollments…</div>}
      </div>
      ...
      <div className="overflow-x-auto shadow-sm rounded-xl">
        <table className="min-w-full text-sm bg-white">
          ...
          <tbody className="text-[#1C1A57]">
            ...
            {currentData.map((row, index) => (
              <tr
                key={`${row.courseId}-${row.userId}-${index}`}
                className="border-b hover:bg-[#F9F9FB]"
              >
                {/* S.No */}
                <td className="px-4 py-3">
                  {(currentPage - 1) * entriesPerPage + index + 1}
                ...
                <td className="px-4 py-3">{row.userName || "-"}</td>
                <td className="px-4 py-3">{getModeLabel(row.mode)}</td>
                <td className="px-4 py-3 text-gray-600">{formatDate(row.enrolledDate)}</td>
              </tr>
            ))}
          </tbody>
        </table>
