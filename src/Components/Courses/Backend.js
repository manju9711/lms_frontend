...
//       try {
//         const res = await axios.get('http://localhost:5000/api/course?category=Backend Development');
//         setCourses(res.data);
//       } catch (error) {
    ...
    const fetchCourses = async () => {
      try {
        const res = await axios.get(
          `http://localhost:5000/api/course?category=Backend Development&userId=${user?.id || ''}`
        );
        setCourses(res.data);
      } catch (error) {
        console.error('Failed to fetch courses:', error);
